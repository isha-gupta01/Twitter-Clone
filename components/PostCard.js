...
const PostCard = () => {
    ...
    const handleSavedPosts = async (tweetId) => {
        try {
            ...
            setIsSaved(prev => ({ ...prev, [tweetId]: data.hasSaved }));
        } catch (error) {
            console.error(error);
        }
    ...
    useEffect(() => {
        ...
        const fetchTweetsAndSaved = async () => {
            try {
                setLoading(true);

                // 1️⃣ Fetch all tweets
                const resTweets = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tweetfetch/tweets`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                if (!resTweets.ok) throw new Error("Failed to fetch tweets");
                const tweetsData = await resTweets.json();
                setData(tweetsData);

                // 2️⃣ Fetch comment counts
                ...
                // 3️⃣ Fetch saved tweets
                const resSaved = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/loggeduser/getSavedTweet`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                if (!resSaved.ok) throw new Error("Failed to fetch saved tweets");
                const savedData = await resSaved.json();
                const savedTweetIds = savedData.savedTweets.savedTweet.map(t => t._id);

                // Map tweet IDs to boolean for easy lookup
                const savedState = {};
                tweetsData.forEach(tweet => {
                    savedState[tweet._id] = savedTweetIds.includes(tweet._id);
                });
                        ...
                        <div className="h-auto ml-8 md:ml-0  justify-center items-center md:justify-between  flex flex-col">
                            ...
                            <ul className="flex text-sm items-center flex-row md:ml-2 gap-5 md:gap-7 mt-4">
                                ...
                                <div className="flex mx-2 cursor-pointer flex-row gap-5 md:gap-10 items-center">
                                    <button onClick={() => handleSavedPosts(item._id)} className="hover:fill-blue-500 w-7 fill-white/50">
                                        {isSaved[item._id] ? (<div className="w-6"><Image src="/bookmark1.svg" width={14} height={14} alt="Saved" /></div>) : (<svg viewBox="0 0 24 24"
                                            aria-hidden="true"
